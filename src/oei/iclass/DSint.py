...
class DSint(OEint):
    ...
    def save_int(self):
        ...
        self.fha.write("    DSint_0 ds(PAx, PAy, PAz, PCx, PCy, PCz, TwoZetaInv, store, YVerticalTemp); \n")
        for i in range(0,6):
    ...
    def save_int_grad(self):
        ...
        self.fhga.write("    FSint_0 fs(PAx, PAy, PAz, PCx, PCy, PCz, TwoZetaInv, store, YVerticalTemp); \n\n")
        ...
        self.fhga.write("#ifdef REG_FS \n")
        for i in range(0,10):
            self.fhga.write("    LOCSTORE(store, %d, %d, STOREDIM, STOREDIM) = fs.x_%d_%d;\n" % (i+10, 0, i+10, 0))
