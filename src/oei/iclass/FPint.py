...
class FPint(OEint):
        ...
        for m in range(0,self.max_m+1):
            ...
            for i in range(0,10):
                for j in range(0,3):
                    ...
                    for k in range(0,3):
                        if params.Mcal[j+1][k] != 0:
                            self.fhd.write("  val = %s * fs_%d.x_%d_%d - %s * fs_%d.x_%d_%d; \n" % (self.PB[k], m, i+10, 0,\
                            self.PC[k], m+1, i+10, 0))
                            ...
                            if tmp_mcal[k] != 0:
                                tmp_mcal[k] -= 1
                                tmp_i=params.trans[tmp_mcal[0]][tmp_mcal[1]][tmp_mcal[2]]
                                self.fhd.write("  val += TwoZetaInv * %f * (ds_%d.x_%d_%d - ds_%d.x_%d_%d); \n" % (params.Mcal[i+10][k], m, tmp_i-1, 0, m+1, tmp_i-1, 0))

                            #self.fhd.write("  LOCSTOREFULL(store, %d, %d, STOREDIM, STOREDIM, %d) = val; \n" % (i+10, j+1, m))
                            self.fhd.write("  store[%d*gridDim.x*blockDim.x+blockIdx.x*blockDim.x+threadIdx.x] = val; \n" % ((i+10)+((j+1)*params.OEI_STORE_DIM_X)+(params.OEI_STORE_DIM_X*params.OEI_STORE_DIM_Y*m)))
                            break
    ...
    def save_int(self):
        ...
        self.fha.write("    FPint_0 fp(PAx, PAy, PAz, PBx, PBy, PBz, PCx, PCy, PCz, TwoZetaInv, store, YVerticalTemp); \n")

        self.fha.write("#ifdef REG_FP \n")
        for i in range(0,10):
            for j in range(0,3):
                self.fha.write("    LOCSTORE(store, %d, %d, STOREDIM, STOREDIM) = fp.x_%d_%d;\n" % (i+10, j+1, i+10, j+1))
